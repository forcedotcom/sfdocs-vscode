...
function fetchFile(metadata: any, file: VFile) {
    if (metadata.src && metadata.src.indexOf('http') > -1) {
        ...
        try {
            const codeFilePath = path.resolve(file.dirname ? file.dirname : '', metadata.src);
...
export function sfdocsCustomPlugin() {
        ...
        return function transformer(tree: any, file: VFile): void {
            visit(tree, ['code'], function (node: any) {
                ...
                if (node.lang === 'sfdocs') {
                    ...
                    if (renderFunctions && renderCodeBlockFn) {
                        if (meta) {
                            const codeContent = fetchFile(meta, file);
                            if (codeContent) {
                                const metadata = new Map();
